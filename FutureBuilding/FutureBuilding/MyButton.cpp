...
HRGN CMyButton::CreateRgnFromBitmap(HBITMAP hBmp, COLORREF color)
{
	if (!hBmp) 
		return NULL;
	BITMAP bm;
...
//
//=============================================================================
BOOL CMyButton::LoadStdImage(UINT normal_id, UINT mask_id, LPCTSTR pType)
{
	m_pStdImage = new CGdiPlusBitmapResource;
	
	bool ret = m_pStdImage->Load(normal_id, pType);

	m_bMask.DeleteObject();
	
	if (mask_id>0)
	{
		m_bMask.LoadBitmap(mask_id);
...
BOOL CMyButton::LoadStdImage(LPCWSTR normal, LPCTSTR mask, LPCTSTR pType)
{
	m_pStdImage = new CGdiPlusBitmapResource;
...
//
//=============================================================================
//BOOL CMyButton::LoadAltImage(UINT id, LPCTSTR pType)
//{
//	m_bHaveAltImage = TRUE;
///	m_pAltImage = new CGdiPlusBitmapResource;
//	return (m_pAltImage->Load(id, pType));
//}
BOOL CMyButton::LoadAltImage(UINT normal_id, UINT mask_id, LPCTSTR pType)
{
	m_bHaveAltImage = TRUE;
	m_pAltImage = new CGdiPlusBitmapResource;
	
	bool ret = m_pAltImage->Load(normal_id, pType);
...
BOOL CMyButton::LoadAltImage(LPCWSTR normal, LPCTSTR mask, LPCTSTR pType)
{
	m_bHaveAltImage = TRUE;
	m_pAltImage = new CGdiPlusBitmapResource;
	
	bool ret = m_pAltImage->LoadFromFile(normal);//, pType);
...
//				
//=============================================================================
HBRUSH CMyButton::CtlColor(CDC* pScreenDC, UINT nCtlColor) 
{
	if(!m_bHaveBitmaps)
	{
		if(!m_pStdImage)
		...
		if (m_dcStd.m_hDC == NULL)
		{
			...
			if (m_dcStdP.m_hDC == NULL)
			{
               ...
               /* CMemDC p;
				p.CreateCompatibleDC(pDC);
				p.StretchBlt(0, 0, rect.Width(), rect.Height(),pDC, 0, 0, SRCCOPY)*/
			/*	m_dcStdP.BitBlt(0, 0, rect.Width(), rect.Height(), p, 0, 0, SRCCOPY);*/
			...
			if(m_dcStdH.m_hDC == NULL)
			{
				...
				float width, height;
				if(m_pStdImage->m_pBitmap)
				{
					width = (float)m_pStdImage->m_pBitmap->GetWidth();
					height = (float)m_pStdImage->m_pBitmap->GetHeight();
				}
...
//=============================================================================
void CMyButton::PaintBtn(CDC *pDC)
{
	CRect rect;
	GetClientRect(rect);
...
//=============================================================================
void CMyButton::EnableToggle(BOOL bEnable)
{
...
//=============================================================================
void CMyButton::SetImage(int type)
{
	m_nCurType = type;
...
//
//=============================================================================
void CMyButton::SetBkGnd(CDC* pDC)
{
	CRect rect, rectS;
	CBitmap bmp, *pOldBitmap;
...
//=============================================================================
void CMyButton::SetToolTipText(UINT nId, BOOL bActivate)
{
...
//=============================================================================
void CMyButton::SetToolTipText(CString spText, BOOL bActivate)
{
